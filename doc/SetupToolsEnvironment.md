...
```
oc policy add-role-to-user system:image-puller system:serviceaccount:3a0694-dev:default --namespace=3a0694-tools
```
...
8. The following instructions are for the build and deployment of the `sbc` component. The build uses the on-cluster `nodejs:10` S2I image to run any scripts from `package.json` which require node. This step produces an artifacts image (sbc-web-artifacts) that used as part of chained build. These artifacts are (from `npm run build`) are then consumed by the NGINX image which is pulled in from the RedHat Container Registry. This results in an image named `sbc-web` that can be deployed.

The deployment mounts a `ConfigMap` containing the necessary NGINX config.

### Build

9. The GitHub Workflow (Actions) will use `oc` to trigger commands on-cluster. This workflow is located [here](../.github/workflows/sbc.yml) in the `.github/workflows` folder of this project.

This workflow is setup to **automatically run** whenever files in these paths are changed:

```yaml
    paths:
      - "mcap-schedule/src/**/*.html"
      - "mcap-schedule/src/**/*.js"
      - "mcap-schedule/package*.json"
```
...
```console
oc process -f mcap-schedule/openshift/templates/build.yaml | \
  oc create -f -
```

### Deploy

10. The deployment for the `sbc` component is straight forward as it has little to no environment variables. The first step in the deploument is to create a `ConfigMap` with the necessary NGINX config:

```console
oc process -f mcap-schedule/openshift/templates/config.yaml | \
  oc create -f -
```
...
```console
oc process -f mcap-schedule/openshift/templates/deploy.yaml \
  -p NAMESPACE=$(oc project --short) \
