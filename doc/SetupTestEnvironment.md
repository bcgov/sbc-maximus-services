...
2. create ./openshift/templates/nsp-sbc-to-maximus-test.yaml  (copy from -dev.yaml)
   change the IP of proxy to test proxy
   the apply using:
```console
oc process -f nsp-sbc-to-maximus-test.yaml \
...
4. allow the test project to pull from tools:
   Go to the test project (oc project 3a0694-test).
```console
